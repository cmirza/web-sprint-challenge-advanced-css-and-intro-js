...
*/
function getArtistByIndex(inputArtists, inputIndex) {
  return `the artist at index ${inputIndex} is ${inputArtists[inputIndex].name}`
}
...
function get20s(inputArtists){
  let artists20s = [];

  for (let i=0; i<inputArtists.length; i++){
    let yearSpread = inputArtists[i].years.split(" - ");
    let birthYear = parseInt(yearSpread[0]);
    let deathYear = parseInt(yearSpread[1]);

    if(birthYear>=1900 && deathYear<=2000){
      artists20s.push(inputArtists[i].name);
    }
  }
  return artists20s;
}
...
function removeArtist(inputArtists, inputIndex) {
  inputArtists.splice(inputIndex, 1);
  return inputArtists.length;
}
...
function addArtist(inputArtists, inputID, inputName, inputYears, inputGenre, inputNationality, inputBio){
  inputArtists.push({
    inputID, 
    inputName, 
    inputYears, 
...
function lotsOfArt(inputArtists){
  let over100 = [];

  for (let i=0; i<inputArtists.length; i++){
    if (inputArtists[i].paintings>100){
      over100.push(inputArtists[i].name);
    }
  }
  return over100;
}
